                ...
                <TableBody>
                  {renderData.filter(item => !deleteData.includes(item.id)).map((row, idx) => (
                    <TableRow key={row.name}>
                      ...
                      <TableCell style={{ border: 'none' }}><Typography variant='subtitle1'>RM {(Number(row.Price) * (row.Quantity)).toFixed(2)}</Typography></TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
        ...
        >
          <Grid item xs={12} sm={8} justify="right" alignItems="right" style={{ marginTop: '10px' }}>
            <div style={{ display: "flex", justifyContent: "right" }}>
              ...
              {checkOutState === false ?
                <Button variant='contained' onClick={() => handlePlaceOrder()} style={{
                  borderRadius: '30px', backgroundColor: selected.length === 0 ? '#D1D1D1' : '#8fb136',
                    ...
                    <Grid item xs={12} sm={4}>
                      ...
                      <Grid item container spacing={2}>
                        <Grid item xs={12} sm={12} style={{ display: 'flex', flexDirection: 'row', justifyContent: 'center' }}>
                          <Button variant='contained' onClick={() => handleCheckOut()} style={{
                            borderRadius: '30px', backgroundColor: selected.length === 0 ? '#D1D1D1' : '#8fb136',
                            color: selected.length === 0 ? 'white' : 'white', textTransform: 'none'
                          }}
                            disabled={selected.length === 0}
